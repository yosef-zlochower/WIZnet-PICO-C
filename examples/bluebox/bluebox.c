 ...
 * - W5500 SPI: SPIO_SCK (GPIO2), SPIO_TX (GPIO3), SPIO_RX (GPIO4), CSn (GPIO1),
 * RSTn (GPIO0)
 * - 1-Wire: GPIO6
 * - LED: GPIO29
...
#define FLASH_TARGET_OFFSET                                                    \
    (1024 * 1024) // Offset for a 4KB sector at the end of flash
...
int main()
{
    ...
    if (gpio_get(CONFIG_BUTTON_PIN) == 0)
    {
        ...
        memcpy(net_config.mac, (uint8_t[]){0x00, 0x08, 0xDC, 0x12, 0x34, 0x56},
               6);
    ...
    else
    {
        ...
        memcpy(g_net_info.mac, (uint8_t[]){0x00, 0x08, 0xDC, 0x12, 0x34, 0x56},
               6);
    ...
    // Open a UDP socket
    if (socket(SOCKET_NUM, Sn_MR_UDP, g_net_info.ip[3], 0) == 0)
    { // Using a local port from IP for simplicity
    ...
    for (;;)
    {
        ...
        if (packet_ready == 1)
        {
            ...
            // Send the UDP packet
            int32_t len = sendto(SOCKET_NUM, packet_buffer, PACKET_SIZE,
